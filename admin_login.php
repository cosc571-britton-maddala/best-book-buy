...
if(isset($_POST["login"]) && !empty($_POST["adminname"])) {
	$login = $_POST["login"];
	...
	if(isset($rows[0])) 
	{
		$_SESSION["admin_logged_in"] = $rows[0];
		echo("<script type='text/javascript'> document.location.href='admin_tasks.php'</script>");
	} else 
	{
		$error = true;
	}
		...
		<div class="row justify-content-center p-5">
			<div class="col-xl-8 col-lg-12">
				<div class="card shadow bg-secondary-subtle">
					<div class="card-header h2 text-center bg-dark text-white"> Admin Login</div>
					<div class="card-body">
						<?php if($error) : ?>
							<div class="row">
								<div class="col"> 
									<div class="alert alert-danger" role="alert">Admin Login was not correct for: <?php echo($adminname) ?> </div> 
								</div>
							</div>
						<?php endif ?>
						<form action="" method="post" class="row g-3">
							<div class="col-md-6">
								<label for="adminname" class="form-label">Adminname<span class="text-danger">*</span></label>
								<input type="text" class="form-control" name="adminname" placeholder="Enter Adminname" aria-label="adminname">
							</div>
							<div class="col-md-6">
